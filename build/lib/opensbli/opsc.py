...
class OPSCCodePrinter(CCodePrinter):
    ...
    def _print_Rational(self, expr):
        if self.constants is not None:
            if expr in list(self.constants.keys()):
                return self.constants[expr]
            else:
                con = 'rc%d' % len(list(self.constants.keys()))
...
def pow_to_constant(expr, constants):
    ...
    for at in expr.atoms(Pow):
        if _coeff_isneg(at.exp) and not at.base.atoms(Indexed):
            if at not in list(constants.keys()):
                constants[at] = 'rinv%d' % len(list(constants.keys()))
...
class OPSC(object):
    ...
    def initialise_constants(self):
        ...
        constant_dictionary = dict(list(zip(self.constants, values)))
        # Sort constants
        sorted_constants = []
    ...
    def sort_constants(self, constant_dictionary, sorted_constants):
        ...
        sorted_constants = [key for key, value in constant_dictionary.items()
        ...
        key_list = [key for key in list(constant_dictionary.keys()) if key not in sorted_constants]

        requires_list = [constant_dictionary[key].atoms(Symbol) for key in key_list]
        zipped = list(zip(key_list, requires_list))
        ...
        for d in dicts:
            for key, value in d.items():
                if key.is_grid:
                    ...
                    if stencil not in list(self.stencil_dictionary.keys()):
    ...
    def declare_stencils(self):
        ...
        for key, value in self.stencil_dictionary.items():
    ...
    def kernel_computation(self, computation, block_number):
        ...
        # Grid-based objects
        grid_based = ([self.ops_header['inputs'] % (self.dtype, inp) for inp in list(computation.inputs.keys()) if inp.is_grid] +
                      [self.ops_header['outputs'] % (self.dtype, inp) for inp in list(computation.outputs.keys()) if inp.is_grid] +
                      [self.ops_header['inputoutput'] % (self.dtype, inp) for inp in list(computation.inputoutput.keys()) if inp.is_grid])

        # Non grid-based objects
        nongrid = ([self.ops_header['inputs'] % (self.dtype, inp) for inp in list(computation.inputs.keys()) if not inp.is_grid] +
                   [self.ops_header['outputs'] % (self.dtype, inp) for inp in list(computation.outputs.keys()) if not inp.is_grid] +
                   [self.ops_header['inputoutput'] % (self.dtype, inp) for inp in list(computation.inputoutput.keys()) if not inp.is_grid])
    ...
    def update_definitions(self, computation):
        ...
        arrays = set([inp for inp in list(computation.inputs.keys()) if inp.is_grid] +
                     [inp for inp in list(computation.outputs.keys()) if inp.is_grid] +
                     [inp for inp in list(computation.inputoutput.keys()) if inp.is_grid])
        constant_arrays = set([inp for inp in list(computation.inputs.keys()) if not inp.is_grid] +
                              [inp for inp in list(computation.outputs.keys()) if not inp.is_grid] +
                              [inp for inp in list(computation.inputoutput.keys()) if not inp.is_grid])
        ...
        self.constants = self.constants.union(constant_arrays).union(constants).union(list(self.rational_constants.values()))\
            .union(const)
        # update the simulation parameters as these are used for writing the constants
        self.simulation_parameters.update(dict(list(zip([str(v) for v in list(self.rational_constants.values())],
